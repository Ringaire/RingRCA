...
async fn handle_tg_message(state: &TgState, msg: TgMessage) {
    ...
    if text == "/start" || text == "/help" {
        let help = "\u{1F43E} *NekoCLI - Telegram*\n\n\
Send any message to chat with the AI agent.\n\n\
Commands:\n\
/help  - Show this\n\
/status - Connection status\n\n\
Messages are routed to a connected NekoCLI.";
    ...
    let session_id = match state.router.pick_cli().await {
        Some(id) => id,
        None => {
            let _ = send_tg_message(&state.http, &state.bot_token, msg.chat.id, "No agent connected. Start NekoCLI and connect first.", None).await;
            return;
        }
    };
